...
$requiredVariables = @(
    ...
    @{Name='BeyondTrustBaseUrl';Value=$BeyondTrustBaseUrl},
    @{Name='BeyondTrustClientId';Value=$BeyondTrustClientId},
    @{Name='BeyondTrustClientSecret';Value=$BeyondTrustClientSecret},
    @{Name='TeamsChannelName';Value=$TeamsChannelName}
)
...
function New-AzureResources {
   ...
   try {
       ...
       # Persist critical information for potential later use
       $persistPath = Join-Path $LocalRepoPath "deployment_info.json"
       $deploymentInfo = @{
           BotId = $appId
           BotPassword = $secret.password
           StorageAccountName = $storageAccountName
           StorageConnectionString = $storageConnString
           WebAppName = $webAppName
           FunctionAppName = $functionAppName
           ResourceGroupName = $ResourceGroupName
       } | ConvertTo-Json

       $deploymentInfo | Set-Content -Path $persistPath -Encoding UTF8

       Write-Host "Deployment information saved to $persistPath" -ForegroundColor Green
...
function Deploy-CodeToAzure {
    ...
    # Copy function files and directories
